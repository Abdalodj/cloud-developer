...
describe('concat', () => {
  it("should concat 'abcd' and 'efgi'", () => {
    const result = concat('abcd', 'efgi');
    expect(result).to.equal('abcdefgi');
  });

  it("should throw an error Empty string", () => {
    expect(() => { concat('abcd', '') }).to.throw('Empty string');
  });
});
